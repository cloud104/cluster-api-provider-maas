...
# Image URL to use all building/pushing image targets
IMAGE_NAME := manager
REGISTRY ?= "us-east1-docker.pkg.dev/tks-staging/cluster-api-provider-maas"
IMG_TAG ?= v1.0.0
CONTROLLER_IMG ?= ${REGISTRY}/${IMAGE_NAME}

# Set --output-base for conversion-gen if we are not within GOPATH
ifneq ($(abspath $(REPO_ROOT)),$(shell go env GOPATH)/src/github.com/cloud104/cluster-api-provider-maas)
...
generate-go:
	...
	$(CONVERSION_GEN) \
		--extra-peer-dirs=github.com/cloud104/cluster-api-provider-maas/api/v1beta1 \
